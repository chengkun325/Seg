...
def focal_loss_with_logits(
    ...
    """
    这个函数用于计算二分类的 Focal Loss。

    reduced_threshold（）等。
    :param output: 任意形状的张量，模型的预测
    :param target: 与输入相同形状的张量
    :param gamma: Focal Loss 的幂因子
    :param alpha: 正负样本权重平衡因子
    :param reduction: 减少操作类型，例如求平均或求和
    :param normalized: 是否计算归一化的 Focal Loss
    :param reduced_threshold: 减少的 Focal Loss 阈值
    :param eps
    :param ignore_index

    函数的计算过程如下：
    将 target 转换为与 output 相同的类型。
    计算 output 的 sigmoid 激活值 p。
    使用二进制交叉熵函数 F.binary_cross_entropy_with_logits 计算交叉熵损失 ce_loss，但不进行减少操作。
    计算 pt，即 p * target + (1 - p) * (1 - target)。
    根据是否存在减少阈值，计算 focal_term。
    如果没有减少阈值，则为 (1.0 - pt).pow(gamma)；
    否则，计算 ((1.0 - pt) / reduced_threshold).pow(gamma)，并将小于阈值的部分置为 1。
    计算最终的 Focal
